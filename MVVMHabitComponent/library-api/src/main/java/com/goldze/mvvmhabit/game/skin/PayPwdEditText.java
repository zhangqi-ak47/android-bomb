...
public class PayPwdEditText extends RelativeLayout{
    ...
    public EditText editText; //文本编辑框
    ...
    public void initShowInput(int bgcolor, int pwdlength, float slpilinewidth, int splilinecolor, int pwdcolor, int pwdsize)
    {
        ...
        params2.setMargins(0, dip2px(context, 0.8f), 0, dip2px(context, 0.8f));
    ...
    public void setLayoutBg(int bgcolor){
        ...
        if(s.length() > 0)
        {
            ...
            for(int i = 0; i < pwdlength; i++)
            {
                if(i < length)
                {
                    ...
                    {
                        char ch = s.charAt(j);
                        textViews[j].setText(String.valueOf("*"));
                    }
                }
                else
                {
